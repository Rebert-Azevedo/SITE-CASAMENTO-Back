...
exports.reserveGift = async (req, res, next) => {
  ...
  if (!nome_reservou || !telefone) {
    return res
      .status(400)
      .json({ message: "Nome e telefone são obrigatórios para reservar." });
  }
  ...
  try {
    ...
    if (giftRows.length === 0 || giftRows[0].status !== "disponível") {
      ...
      return res
        .status(409)
        .json({ message: "Presente não está disponível para reserva." });
    }
...
exports.createGift = async (req, res, next) => {
  ...
  if (!nome) {
    return res
      .status(400)
      .json({ message: "O nome do presente é obrigatório." });
  }
  try {
...
exports.deleteGift = async (req, res, next) => {
  const { id } = req.params;
  try {
    const [result] = await pool.execute("DELETE FROM presentes WHERE id = ?", [
      id,
    ]);
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Presente não encontrado." });
